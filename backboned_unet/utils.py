...
def dice_score(input, target, classes, ignore_index=-100):
    ...
    # converting to onehot image with class channels
    onehot_target = torch.LongTensor(target.shape[0], classes, target.shape[-2], target.shape[-1]).zero_().to(device)
...
class DiceLoss(torch.nn.Module):
    ...
    def __init__(self, classes, loss_mode='negative_log', ignore_index=255):
